...
### Step 3: Check for any context from recent nightly run slack threads
- There should be two slack threads created every morning at approximately 02:30 UTC, one for RHOAI and one for ODH. Read back through the last 3 threads in the channel and look for any context that may be relevant to the current nightly run.
